        ...
        <div className={styles['channel-detail-page']}>
            ...
            <form>
                <AdminPageSection label='기본 정보' collapsable>
                    ...
                    <TextInput label='채널 이름' placeholder='예) 네이버' name='name' maxLength={20} initialValue={channel?.name || ""} required />
                    <TextInput label='수수료율' placeholder='% 수치 그대로 입력하세요. 예) 9.8% => 9.8' name='fee' min={0} max={100} initialValue={(channel?.fee || "").toString()} type='number' step={0.01} required />
                </AdminPageSection>
                <AdminPageSection label='엑셀 정보' collapsable>
                    <TextInput label='엑셀 파일명' placeholder='정규식 입력' name='excelFileName' maxLength={100} initialValue={channel?.excelFileName || ""} />
